...
`;
    ...
    expect(
      UtilsI18nHtml.extractGettextTranslateFromHtml(html).length,
    ).to.be.above(0);
  });

  it('should not extract when translate inside class name', () => {
    const html = `
 <button
    mat-icon-button
...
`;
    ...
    expect(UtilsI18nHtml.extractGettextTranslateFromHtml(html).length).to.be.eq(
      0,
    );
  });
